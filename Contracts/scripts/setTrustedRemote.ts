...
const main = async () => {
  ...
  // Optimism
  let trustedRemote = solidityPacked(['address', 'address'], [remote_opt, local_opt]);

  let trustedRemoteBase = solidityPacked(['address', 'address'], [remote_base, local_base]);
  ...
  const call = await contract.setTrustedRemote(remoteChainID_opt, trustedRemote, { gasPrice: '350000' });
